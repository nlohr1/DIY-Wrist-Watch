...
boolean PIN_D2 = 1; //read binary 0 or 1
 ...
 if (mode == 1) { //Calendar:
  ...
  //Day-Of-Week setting:
   ...
   if (mode == 6) { // (incrementing from year 00 to 99, jumping at 1st Button-D8-press to "19")
    if (flash) { display.drawRect(85,0,27,9,WHITE); } //Display rectangle cursor every other display update (5Hz blink)
    if ((BUTTON_D8 == 0) && (!flash)) { //Update setting at 5Hz rate if button held down
  ...
  if (BUTTON_D8 == 0) {  //if Button-D8 (=PB0) was pressed, the Falling-Interrupt function (ISR) sets the Button-STATE "BUTTON_D8" to LOW // (!digitalRead(8) replaced per Pin-Change-Interrupt !
    ...
    if ((chng_Flag == 1) && (chng_Count >= 1)) { //wait 1 loops  of ≈ 100ms (to stabilize Button-D8 bouncing)
...
2.) IMPORTANT: In menu Tools/Board/Arduino AVR Boards ⇒ choose "Lilipad Arduino" (= chip ATmega328 running with internal-8MHz) !!! NOT Arduino.Uno or -Mini running at 16 MHz!
